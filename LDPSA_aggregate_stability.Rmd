...
Fixed-effects regressions assume a non-hierarchical data structure; i.e., instances where data points are not nested or grouped in higher order categories (e.g. soil profiles within landscapes). We initially model the values in air (Δ air) and in water (Δ water) to assess the population-level effects of the included covariates.
...
```{r, fig.align ="center", fig.show='hold', out.width="50%", fig.cap="**Figure 5:** Fitted vs observed cumulative link index values in air or water relative to dispersion in the calgon with sonication (`c4`) treatment based on fixed effects models."}
...
Fixed-effects models such as those shown in section ... fail to recognize the intraclass correlation effects of clustering. Here is a simple example:
...
Alternatively in mixed-effects (or multilevel) models the data being analysed are drawn from a hierarchy of different (sub)populations whose differences relate to that hierarchy. In this context, a mixed effects model that takes site (landscape) level differences can be as expressed as:
...
Note that the expression contains both fixed-effects fits (from the fixed-effects regression above), as well as two random-effects. The $u_i$ term is a site-level random effect. It measures the difference between the average DFI value of the *i*^th^ site and the average DFI value in the population of sites. The $e_{ij}$ term is a profile-specific random effect. It expresses the difference between the *j*^th^ profile DFI value from the average DFI of the *i*^th^ site. We can calculate these differences and plot them with the following chunks.

```{r}
# mixed-models with site-level random effects
...
```{r, fig.align ="center", out.width="60%", fig.cap="**Figure 6:** Profile-level fitted vs observed DFI values. The blue lines represent the 95% quantile interval of the measured DFIs."}
