...
The data needed for running this notebook can be obtained from the Open Science Framework [here](https://osf.io/yr8ua/). We shall use topsoil (0-20 cm) and co-located subsoil (20-50 cm) data that were collected as part of the AfSIS project. The data were sampled in 60, 10 × 10 km  sentinel landscapes ([Vagen et. al., 2015](https://www.sciencedirect.com/science/article/pii/S0016706115300082)) in proportion to their occurrence in the major [Köppen-Geiger](http://koeppen-geiger.vu-wien.ac.at/) climate zones of Africa excluding deserts, urban and other non-photosynthetically active land areas. Each sentinel site represents a 100 km^2^ area, with a total of 160 soil profile sampling locations within each sentinel landscape. A randomly selected ~10% subset of (955 / 9600) soil profiles were analyzed with LDPSA.
...
Note that an identical chunk is replicated for the samples that were dispersed in Calgon. This is not included in the html here to keep the text concise. However, it is included in the actual R-markdown doc on [Github](). The chunks generate ternary plots using the `ggtern` package in R-markdown. The figures appear convincing in that in that as a whole they show that the particle dispersion in both water and Calgon work in the anticipated direction in response to the combination of the dispersion media and sonication treatments.
...
```{r, echo = FALSE, fig.align = "center", fig.show = 'hold', out.width = "50%", fig.cap = "Ternary plots of the particle size distributions of LDPSA samples dispersed in either in water (top) or Calgon (bottom). Color coded by sonication time (`mwd`, left), and aggregate stability index (`stime`, right)."}
...
The obvious message that is conveyed by the graph above is that dispersion in Calgon coupled with 4 minutes of sonication is really effective in destroying large (sand sized) soil aggregates when compared to simple dispersion in water. The two isometric logratios of the particle size data (`ilr1` & `ilr2`) demonstrate this treatment endpoint pattern quite well. 

## Simple aggregate stability / dispersion index
...
# Landscape-level soil aggregate stability
