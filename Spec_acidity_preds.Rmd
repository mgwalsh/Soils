...
Note that I have also included pH (water) in the calibration features beacuse it is routinely, quickly and cheaply measured in most soil labs. It is also closely related to Hp.
...
There are many ways to quantify the uncertainty inherent in these predictions. We take a simple but quite robust approach here using quantile regression with ([quantreg](https://cran.r-project.org/web/packages/quantreg/quantreg.pdf)). I am mainly interested in the initial spread of the spectral ensemble predictions (sensu, their 95% probable intervals).

```{r, warning = FALSE}
adata$pls <- predict.train(llist$pls, adata)
adata$glm <- predict.train(llist$glmnet, adata)
adata$xbl <- predict.train(llist$xgbLinear, adata)
adata$rfo <- predict.train(tlist$rf, adata)
adata$xbt <- predict.train(tlist$xgbTree, adata)
adata$cub <- predict.train(tlist$xgbLinear, adata)
adata$st <- predict(st, adata)
```

```{r, fig.align = "center", fig.cap = "Quantile regression plot of modeled vs observed reserve acidity (Hp). The blue lines are the 2.5% and 97.5% quantile regression estimates."}
