...
set.seed(seed)
tg <- trainControl(method = "cv", number = 10, allowParallel = TRUE, savePredictions="final")
...
The 10-fold cross-validation RMSE's of the calibration llist models are as follows: 

```{r}
options(digits = 3)
llist_results <- data.frame(
                 pls = min(llist$pls$results$RMSE),
                 gln = min(llist$glmnet$results$RMSE),
                 xbl = min(llist$xgbLinear$results$RMSE))
print(llist_results)
```

The next chunk fits 3 additional, tree/forest based MLAs that use the that use the spectral principal component scores (spc = 20). You can learn more about how they work in R, from the following links at: [randomForest](https://www.rdocumentation.org/packages/randomForest/versions/4.6-14/topics/randomForest), [xgboost](https://www.kaggle.com/rtatman/machine-learning-with-xgboost-in-r/)) and [Cubist](https://cran.r-project.org/web/packages/Cubist/Cubist.pdf)).
...
```{r}
options(digits = 3)
tlist_results <- data.frame(
                 rfo = min(tlist$rf$results$RMSE),
                 xbt = min(tlist$xgbTree$results$RMSE),
                 cub = min(tlist$cubist$results$RMSE))
print(tlist_results)
```
