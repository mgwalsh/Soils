...
As can be seen from summary output above the `cubist` predictions outweigh the influence of all of the other model predictions in the `st` ensemble on the validation set. This raises the question if a simpler, more pasimonious model, which just uses the `cubist` MLA with the spectral principal components plus pH would be a potential alternative for operational diagnoses of Hp. This would certainly reduce computing times and might be implemented in an online system.
...
```{r}
stQ <- rq(Hp~st, tau=c(0.05,0.5,0.95), data = adata) ## quantile regression fit
print(stQ)
```

```{r, fig.align = "center", fig.cap = "Quantile regression fits of modeled vs observed reserve acidity (Hp). The blue lines are the 5% and 95% quantile regression estimates."}
par(pty="s", mar=c(4,4,1,1))
