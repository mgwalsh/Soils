...
As can be seen from summary output above panel, the `cubist` predictions outweigh the influence of all of the other model predictions in the `st` ensemble on the validation set, with much smaller contributions from the `glmnet` and `xgbLinear` predictions. This raises the question if a simpler, more parsimonious model, which only uses `cubist` with the spectral principal components plus pH would be a potential alternative as an operational diagnoses of Hp. This would certainly reduce computing times. I explore this in the next section.
...
```{r, fig.align = "center", figures-side, fig.show="hold", out.width="50%", fig.cap = "Quantile regression fits of modeled vs observed reserve acidity (Hp). The blue lines are the 5% and 95% quantile regression estimates. On the left is the stacked model predictions, `st`. On the right are the `cubist` based predictions."}
