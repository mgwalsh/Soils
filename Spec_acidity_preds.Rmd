...
Most soil testing laboratories and extension services make lime recommendations based on calibrations of pH measured before and after the addition of a pH buffer solution (e.g., Ca(OH)~2~). *Soil-test-based* lime recommendations (LR, in kg ha^-1^ CaCO~3~ equivalents, see our schema for this in the figure below) are subsequently adjusted to account for incorporation depth, liming material CCE, lime type (e.g. calcitic vs dolomitic), lime moisture content, parent material and time since previous lime application. The associated calculations are pretty straightforward (see e.g., the excellent guidance provided by [PSU extension material](https://extension.psu.edu/soil-acidity-and-aglime) about this)). The AfSIS approach is also pretty straightforward (and simplistic) currently ... see the schema directly below.  

```{r, echo=FALSE, fig.align="center", fig.cap="AfSIS schema for field-crop, soil-test based (topsoil, 0-20 cm) Aglime recommendations (LR, in kg ha^-1^ CCE).", out.width = '60%'}
knitr::include_graphics("./soil_acidity/Figures/Lime requirement.png")
```

While this is just another liming recommendation schema (i.e., informed opinion) that farmers could take or leave on the basis of soil tests, there are of course both much more complex and more dynamic social/economic/environmental interactions at play e.g., between the effects of applications of lime, nitrogen fertilizers, other nutrient remediation measures, existing cropping systems, farmer investment risk aversion, water-use and greenhouse gas emissions that should be considered. These [additionalities](https://ghginstitute.org/wp-content/uploads/2015/04/AdditionalityPaper_Part-1ver3FINAL.pdf) will require systematic and spatially/temporally representative surveys and field trials to monitor any potential socio-economic constraints and also any potentially associated environmental side effects. Systematic agronomic field trials should also be used to (in)validate and challenge any of the inherent assumptions that are made in using Hp for making soil-test-based liming recommendations to farmers. However, that is also true for any other purely soil-test-based procedures that might be used for issuing such recommendations. 
