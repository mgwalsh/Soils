...
The 10-fold cross-validation RMSE's of the calibration alist models are as follows: 

```{r}
options(digits = 3)
alist_results <- data.frame(
                 pls = min(model_list$pls$results$RMSE),
                 glm = min(model_list$glmnet$results$RMSE),
                 xbl = min(model_list$rf$results$RMSE))
print(alist_results)
```
