...
As can be seen from summary output panel above, the `cubist` predictions outweigh the influence of all of the other models in the `st` ensemble predictions on the validation set, with much smaller contributions from the `glmnet` and `xgbLinear` models. This raises the question if a simpler, more parsimonious model, which only uses `cubist` with the spectral principal components plus pH would be a potential alternative to provide operational diagnoses of Hp. This would certainly reduce computing times and also lend itself automation. I explore this in the next section.
