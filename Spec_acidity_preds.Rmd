...
* The main note of caution is that soil-test-based liming recommendations (among other agronomic recommendations) should always be contextualized within specific crop production environments. Simple soil tests are basically not enough to provide useful and reliable extension advice to farmers.

* Computationally, the calibration and validation code (see on [Github](https://github.com/mgwalsh/Soils/blob/master/Spec_acidity_preds.Rmd)) for this notebook is somewhat challenging in standard R ... and requires a bit of patience because it runs relatively slowly on a *normal computer* (8 core, 16 Gb ... doable in ~1 hour). I will try to update both the code and to include cloud-based and/or GPU solutions for larger datasets in the future as needed and/or requested. 
