...
cnls$xCEC <- cnls$m3Ca/200+cnls$m3Mg/120+cnls$m3K/390+cnls$m3Na/230 ## calculates extractable (xCEC)  
...
Nontheless, as can be seen from summary output above the `cubist` predictions outweigh the influence of all of the other model predictions in the `st` ensemble on the validation set. This raises the question if a simpler, more pasimonious model, which just uses the `cubist` MLA with the spectral principal components plus pH would be a potential alternative for the operational diagnoses of Hp. This would certainly reduce computing times and might even be implemented in an online system. I explore this in the next section.
...
stQ <- rq(Hp~st, tau=c(0.05,0.5,0.95), data = adata)
cuQ <- rq(Hp~cub, tau=c(0.05,0.5,0.95), data = adata)
```

```{r, fig.align = "center", fig.cap = "Quantile regression fits of modeled vs observed reserve acidity (Hp). The blue lines are the 5% and 95% quantile regression estimates. On the left is the stacked model `st`. On the right are the overall `cubist` based predictions."}
par(pty="s", mar=c(4,4,1,1))
...
# Notebook summary & recommendations
