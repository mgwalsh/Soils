...
Just by visual inspection of the 2 plots, they do not appear to be much different. Note however, that this is somewhat atypical and will not be the case for most other soil properties. The general rule is that a properly calibrated and validated ensemble model will not perform any worse than the best constituent model in the stack, `st` ... but sometimes much better. This statement also describes the example used in the this notebook with the ensemble having a slightly lower (neglible) validation RMSE the `cubist` model. If you have run all of the R chunks in this notebook you can always check that statement. If all went well, the prediction file to check should be at `./soil_acidity/Results/Hp_preds.csv` in your working directory. 
