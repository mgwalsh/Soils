...
knitr::include_graphics("./soil acidity/Figures/training_validation.png")
```
...
# Liming rate recommendations

Lime application is recommended if soil pH is below the optimum range for any of the crops in the cropping system or crop rotation. The minimum pH of the soil is determined by the crop type or crop species with the highest soil pH requirement. This is typically calculated for a rolling 3-year period. To adjust soil pH to a target pH value for a specific cropping system, one must know both the current soil pH and its reserve acidity. 
Most soil testing laboratories make lime recommendations based on calibrations of pH measured before and after the addition of a pH buffer solution (e.g. Ca(OH)_2_). *Soil test based* lime recommendations (CCE, in kg ha^-1^ CaCO_3_ equivalents, see our schema for this in the figure below) are subsequently adjusted to account for incorporation depth, liming material CCE, lime type (e.g. calcitic vs dolomitic), lime moisture content, parent material and time since previous lime application. The associated calculations are straightforward (see e.g., the excellent [PSU extension material](https://extension.psu.edu/soil-acidity-and-aglime) around this.
...
However, there are also both more complex and more dynamic economic/environmental interactions between the applications of lime, nitrogen fertilizers, other nutrient remediation measures, cropping systems and greenhouse gas emissions that should be considered. These require systematic field trials to monitor any potential socio-economic constraints and any potentially associated environmental side effects.
...
* At the other end of the pH scale the identical workflows can be followed to diagnose e.g., salinity and sodicity levels and certain micro-nutrient deficiencies of cropland soils, which are also prevalent in many parts of Africa. I'll provide provide an additional notebook around these issues in the near future.
